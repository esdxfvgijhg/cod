...
% 	fig(2) = figure(2);
	...
	for SF = SF_min:SF_max  % build the base upchirp
		mu = +1;
		fc = 0;
		phi0 = 2*pi*rand;
		...
		for SF_int = SF_int_min:SF_int_max
			...
			while SINR_idx<=numel(SINR_dB) && seconds+toc<=seconds_budget
				...
				bit_errors_tot = 0;
				trials = 0;
				while rx_errors_tot < rx_errors_threshold && seconds+toc<seconds_budget && trials<max_trials
                    rx_errors_tot
					% random source
					...
					% fft based demodulation (assumes perfect synchronization)
        ...
        legend('PER','SER','BER','Location','Best');
			...
			for i = 1:SINR_idx
				if y_axis_BER(i)>BER_target && y_axis_BER(i+1)<BER_target
					fprintf('SF_ref:%d SF_int:%d SIR for BER=%g: %g [dB]\n', SF,SF_int,BER_target,x_axis(i)+(x_axis(i+1)-x_axis(i)) /(log10(y_axis_BER(i+1))-log10(y_axis_BER(i))) *(log10(BER_target)-log10(y_axis_BER(i))));
				end
			end
		end
