...
def negamax(game, depth, origDepth, scoring, alpha=+inf, beta=-inf, tt=None):
    ...
    """
    alphaOrig = alpha
    lookup = None if (tt is None) else tt.lookup(game)  # Is there a transposition table and is this game in it ?
    if lookup != None:
        if lookup["depth"] >= depth:  # The game has been visited in the past
    ...
    if lookup != None:
        possible_moves = game.possible_moves()  # Put the supposedly best move first in the list
    ...
    if tt != None:
        ...
        tt.store(game=state, depth=depth, value=bestValue, move=best_move, flag=UPPERBOUND if (bestValue <= alphaOrig) else (LOWERBOUND if (bestValue >= beta) else EXACT))
    return bestValue
...
class Negamax:
    ...
    def __call__(self, game):
        ...
        """
        scoring = self.scoring if self.scoring else (lambda g: g.scoring())  # horrible hack
        self.alpha = negamax(game, self.depth, self.depth, scoring, -self.win_score, +self.win_score, self.tt)
        return game.ai_move
