...
if __name__ == "__main__":
    ...
    with open(feature_map_file, "r") as f:  # Load the feature map
        feature_map = pickle.load(f)
    label_words = [x["object_class"] for x in feature_map]  # Extract feature vectors and the labels
    dim_size = feature_map[0]["feature_vector"].shape[1]
    X = [np.reshape(x["feature_vector"], (dim_size,)) for x in feature_map]
    erf = ERFTrainer(X, label_words)  # Train the Extremely Random Forests classifier
    if args.model_file:
