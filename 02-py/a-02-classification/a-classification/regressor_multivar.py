...
input_file = "data_multivar_regr.txt"  # Input file containing data
data = np.loadtxt(input_file, delimiter=",")  # Load the data from the input file
X, y = data[:, :-1], data[:, -1]
num_training = int(0.8 * len(X))  # Split data into training and testing
num_test = len(X) - num_training
X_train, y_train = X[:num_training], y[:num_training]  # Training data
X_test, y_test = X[num_training:], y[num_training:]  # Test data
linear_regressor = linear_model.LinearRegression()  # Create the linear regressor model
