...
def mt(game, gamma, depth, origDepth, scoring, tt=None):
    ...
    if lookup != None and lookup["depth"] >= depth:
        lowerbound, upperbound = lookup["lowerbound"], lookup["upperbound"]  # The game has been visited in the past
        if lowerbound > gamma:
            if depth == origDepth:
    ...
    if tt != None:
        if depth > 0 and not game.is_over():
            ...
            tt.store(game=game, lowerbound=lowerbound, upperbound=upperbound, depth=depth, move=best_move)
    return best_value
