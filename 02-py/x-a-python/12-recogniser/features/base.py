...
import numpy  # Author: James Lyons 2012
from features import sigproc
from scipy.fftpack import dct

try:  # make it python3.x compatible
    xrange(1)
except:
    xrange = range
...
def logfbank(signal, samplerate=16000, winlen=0.025, winstep=0.01, nfilt=26, nfft=512, lowfreq=0, highfreq=None, preemph=0.97):
...
def ssc(signal, samplerate=16000, winlen=0.025, winstep=0.01, nfilt=26, nfft=512, lowfreq=0, highfreq=None, preemph=0.97):
...
def get_filterbanks(nfilt=20, nfft=512, samplerate=16000, lowfreq=0, highfreq=None):
    ...
    #  from Hz to fft bin number # our points are in Hz, but we use fft bins, so we have to convert
...
def lifter(cepstra, L=22):
    ...
    if L > 0:
        ...
        return cepstra  # values of L <= 0, do nothing
