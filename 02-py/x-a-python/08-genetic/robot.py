...
def eval_func(individual):
    global robot, pset
    routine = gp.compile(individual, pset)  # Transform the tree expression to functionnal Python code
    robot.run(routine)  # Run the generated routine
...
def create_toolbox():
    ...
    toolbox.register("expr_init", gp.genFull, pset=pset, min_=1, max_=2)  # Attribute generator
    toolbox.register("individual", tools.initIterate, creator.Individual, toolbox.expr_init)  # Structure initializers
...
if __name__ == "__main__":
    ...
    max_moves = 750  # Define the maximum number of moves
    robot = RobotController(max_moves)  # Create the robot object
    toolbox = create_toolbox()  # Create the toolbox
    with open("target_map.txt", "r") as f:  # Read the map data
        robot.traverse_map(f)
    population = toolbox.population(n=400)  # Define population and hall of fame variables
    hall_of_fame = tools.HallOfFame(1)
    stats = tools.Statistics(lambda x: x.fitness.values)  # Register the stats
    stats.register("avg", np.mean)
    stats.register("std", np.std)
    stats.register("min", np.min)
    stats.register("max", np.max)
    probab_crossover = 0.4  # Define parameters
    probab_mutate = 0.3
    num_generations = 50
    algorithms.eaSimple(population, toolbox, probab_crossover, probab_mutate, num_generations, stats, halloffame=hall_of_fame)  # Run the algorithm to solve the problem
