...
np.random.seed(7)  # Set the seed for random number generator
X, y = make_circles(n_samples=500, factor=0.2, noise=0.04)  # Generate samples
pca = PCA()  # Perform PCA
X_pca = pca.fit_transform(X)
kernel_pca = KernelPCA(kernel="rbf", fit_inverse_transform=True, gamma=10)  # Perform Kernel PCA
X_kernel_pca = kernel_pca.fit_transform(X)
X_inverse = kernel_pca.inverse_transform(X_kernel_pca)
class_0 = np.where(y == 0)  # Plot original data
...
plt.figure()  # Plot PCA projection of the data
...
plt.figure()  # Plot Kernel PCA projection of the data
...
plt.xlabel("1st principal component")
plt.ylabel("2nd principal component")
plt.figure()  # Transform the data back to original space
