...
X_train = np.array(X[:num_training])  # X_train = np.array(X[:num_training]).reshape((num_training,1)) # Training data
y_train = np.array(y[:num_training])
X_test = np.array(X[num_training:])  # X_test = np.array(X[num_training:]).reshape((num_test,1)) # Test data
...
sgd_regressor = linear_model.SGDRegressor(loss="huber", n_iter=50)  # Stochastic Gradient Descent regressor
