...
genre_cols = ["unknown", "Action", "Adventure", "Animation", "Children's", "Comedy", "Crime", "Documentary", "Drama", "Fantasy", "Film-Noir", "Horror", "Musical", "Mystery", "Romance", "Sci-Fi", "Thriller", "War", "Western"]  # Genre columns
gr = np.zeros(NO_OF_GENRES)  # Initialize numpy arrays
gf = np.zeros(NO_OF_GENRES)
rgr = np.zeros(NO_OF_GENRES)
rgf = np.zeros(NO_OF_GENRES)
mrgf = np.zeros(NO_OF_GENRES)
# gr - genre_rating # tr - total_ratings
# rgr - realtive_genre_ratings # gf - genre_frequency
# mrgf - modified_relative_genre_frequency # rgf - relative_genre_frequency
def total_rating(i):  # gim - genre interestingness measure
...
def gim_final(user_movies, i):
    ...
    rgr = relative_genre_rating(gr, tr)  # gf = genre_frequency(movies)
    mrgf = modified_relative_genre_frequency(movies, tf)  # rgf = relative_genre_frequency(gf, tf)
    nf = 5.0
    for i in range(0, NO_OF_GENRES):
