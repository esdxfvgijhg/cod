...
update_field            (GtkSingleSat *ssat, guint i)
{
	...
	printf ("lat %.2f\302\260%c\t", number, hmf);
    ...
    case SINGLE_SAT_FIELD_LON:
        ...
        buff = g_strdup_printf ("%.2f\302\260%c", number, hmf);
        printf ("lon %.2f\302\260%c\t", number, hmf);
        break;
...
void satPrint (sat_t *sat)
{

        
        printf ("lat %6.2f\t", sat->ssplat);
        printf ("lon %6.2f\t", sat->ssplon);
        printf ("alt %6.2f\t", sat->alt);
        printf ("dis %.0f\t", sat->range);
        printf ("spe %.3f\n", sat->range_rate);
	printf ("\n");
}
