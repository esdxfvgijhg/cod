...
void loop(void) {
  ...
  if (radioON && !receivedFromSerial) {
    e = 1;
    e = sx1272.receivePacketTimeout(MAX_TIMEOUT);PRINT_CSTSTR("%s", "Receive:    ") ;PRINT_VALUE("%d", e);PRINTLN;                                                      
    ...
    if (!e) {
      ...
      PRINT_CSTSTR("%s", "Data: ");PRINT_VALUE("%d", sx1272.packet_received.data);PRINTLN;FLUSHOUTPUT;
