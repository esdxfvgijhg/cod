...
void loop(void) {
  ...
  if (receivedFromSerial || receivedFromLoRa) { //cmd
    i = 0;
    if (cmd[i] == '/' && cmd[i + 1] == '@') {
  ...
  else {//if not defined ARDUINO && defined DOWNLINK
...
//          if (document["MIC0"].IsString() && document["MIC0"] != "") { //ifdef INCLUDE_MIC_IN_DOWNLINK
...
//          }
          e = sx1272.sendPacketTimeout(document["dst"].GetInt(), (uint8_t *)document["data"].GetString(), document["data"].GetStringLength(), 10000); //finsi downlink
