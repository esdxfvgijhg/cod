...
plot_aghiles <- function(gnet, 
  ...
  coords <- layout.davidson.harel(gnet)
  ...
  node_unique_data <- signif(c(min(node_unique_data), mean(node_unique_data), max(node_unique_data)),2)
  node_unique_color <- sort(node_color_function(length(node_unique_data)), decreasing = T)
  legend("bottomright", 
         legend= node_unique_data, 
         text.col= node_unique_color, 
...
format <- formatter("span", style = x ~ style(color = ifelse(x > -800 & x < 800, "green", ifelse(x < -800 & x > 800, "red", "black"))))
