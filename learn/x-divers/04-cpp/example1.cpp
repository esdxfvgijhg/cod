...
int main(int argc, char **argv) {                                               //The application code
  ...
  if (test) {
    ...
    P->set_type(DISCRETE);                                                      //Definition of the type of the transition structure the type is not defined by default for a sparse matrix
    ...
    SimulationResult *simRes1 = c1->SimulateChainDT(n, false, true, false);     //Creating a simulationResult object to save results of simulated chain.
    printf("Trajectory \n");                                                    //Write the trajectory of the marmote in the terminal stdout
    simRes1->writeTrajectory(stdout, "standard");
    printf("\n");
    delete simRes1;                                                             //Destruct the simulation object.
    delete c1;                                                                  //Destruct the chain in order to free the allocated memory.
    free(probas);                                                               //NB the object P does not need to be deleted: this is done during the destruction of c1.Destruct what remains
  } else {
...
bool parseParameters(int argc, char **argv, long int *var, double *prob1, double *prob2, double *prob3) {
  bool test = true;
