...
def build_arg_parser():
    parser = argparse.ArgumentParser(description="Extract features from a given  set of images")
    parser.add_argument("--data-folder", dest="data_folder", required=True, help="Folder containing the training images organized in subfolders")
    parser.add_argument("--codebook-file", dest="codebook_file", required=True, help="Output file where the codebook will be stored")
    parser.add_argument("--feature-map-file", dest="feature_map_file", required=True, help="Output file where the feature map will be stored")
    parser.add_argument(
        "--scaling-size", dest="scaling_size", type=int, default=200, help="Scales the longer dimension of the image down  to this size.",
    )
    return parser
...
def get_feature_map(input_map, kmeans, centroids, scaling_size):  # map them to the corresponding object classes # Extract features from the input images and
    feature_map = []
    for item in input_map:
        temp_dict = {}
...
def compute_sift_features(img, keypoints):  # Extract SIFT features
    if img is None:
...
def resize_image(input_img, new_size):  # while maintaining the aspect ratio # Resize the shorter dimension to 'new_size'
...
if __name__ == "__main__":
    ...
    training_data = load_training_data(data_folder)  # Load the training data
    print "====== Building visual codebook ======"  # Build the visual codebook
    kmeans, centroids = FeatureBuilder().get_codewords(training_data, scaling_size)
    if args.codebook_file:
        with open(args.codebook_file, "w") as f:
            pickle.dump((kmeans, centroids), f)
    print "\n====== Building the feature map ======"  # Extract features from input images
