...
int main(int argc, char **argv) {
  ...
  c1->set_generator(P1);  // Assign the transition matrices to chains (respectively).
  c2->set_generator(P2);
  c3->set_generator(P3);
  //////////////////////////////////////////////////////////////////////////
...
// Procedure for reading the command line data and storing it in variables. Returns true if the format of the arguments is valid (not exceeding limits, no unallowed characters). The input is: (number of steps n) (initial distribution p1 p2 p3 p4 p5 p6 p7 p8 p9)
void analysis(MarkovChain *mc) {
  ...
  fprintf(stdout, "number = %ld\n", com.size());
  fprintf(stdout, "list = ( ");
  for (std::vector<std::vector<int> >::iterator iter = com.begin(); iter != com.end(); iter++) {
    fprintf(stdout, "[ ");
    for (std::vector<int>::iterator itr = (*iter).begin(); itr != (*iter).end(); itr++) {
  ...
  fprintf(stdout, "number = %ld\n", rec.size());
  fprintf(stdout, "list = ( ");
  for (std::vector<std::vector<int> >::iterator iter = rec.begin(); iter != rec.end(); iter++) {
    fprintf(stdout, "[ ");
    for (std::vector<int>::iterator itr = (*iter).begin(); itr != (*iter).end(); itr++) {
      fprintf(stdout, "%d ", *itr);
    }
